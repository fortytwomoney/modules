use autocompounder::kujira_tx::{encode_query_supply_of, encode_msg_create_denom, encode_msg_burn, encode_msg_mint, format_tokenfactory_denom, MSG_CREATE_DENOM_TYPE_URL, MSG_MINT_TYPE_URL, MSG_BURN_TYPE_URL, DENOM_PARAMS_PATH, SUPPLY_OF_PATH};
use cw_orch::{prelude::{
    networks,
    Daemon, TxHandler, queriers::{DaemonQuerier, Bank, Node},
}, daemon::{TxBuilder, Wallet, CosmTxResponse, DaemonError}};
use cosmrs::{rpc::{endpoint::{abci_query, broadcast::tx_async::Response}, Client, HttpClient}, tendermint::{serializers::bytes::base64string, block::Height}, tx::{Msg, Raw, Body}, Any, proto::cosmos::tx::v1beta1::TxBody, Coin};
...
pub fn denom_query_msgs() {
    ...
    let supply_of_coin: String = response.value.into_iter().filter_map(|val|{
    ...
    let response = rt.block_on(
        client.abci_query(
            Some(DENOM_PARAMS_PATH.to_string()),
            vec![],
            None,
            true,
        )
