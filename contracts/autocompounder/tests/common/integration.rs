...
pub fn test_deposit_assets<Chain: CwEnv, Dex: DexInit>(
    ...
    vt_total_supply += vault.assert_expected_shares(
        prev_lp_amount,
        vt_total_supply,
        prev_vt_balance,
        user1_addr,
...
pub fn redeem_deposit_immediately_with_unbonding<Chain: CwEnv, Dex: DexInit>(
    ...
    let rest_redeem_amount = total_redeem_amount - redeem_amount;
...
pub fn deposit_fees_fee_token_and_withdraw_fees<Chain: CwEnv, Dex: DexInit>(
    ...
    let received_fee = vault
        .chain
        .bank_querier()
        .balance(commission_addr, Some(EUR.to_string()))
        .unwrap()
        .first()
        .unwrap()
        .amount;
    ...
    assert_that!(claims
        ...
        .u128())
    .is_equal_to(redeem_amount - expected_withdrawal_fee);
...
pub fn compound_reward_distribution<Chain: CwEnv, Dex: DexInit>(
    ...
    assert!(
        new_lp_tokens > expected_new_lp_tokens,
        "New LP tokens are less than expected"
    );
...
pub fn vault_token_inflation_attack<Chain: CwEnv, Dex: DexInit>(
    ...
    vault.mint_lp_tokens(
        minter,
        minter_addr,
        vec![owner_addr, attacker_addr],
        user_deposit,
    )?;
    ...
    assert_eq!(
        attacker_vault_token_balance,
        10u128.pow(DECIMAL_OFFSET),
        "Attacker vault token balance mismatch"
    );
    ...
    assert_eq!(
        lp_staked_after_donation,
        attacker_donation + attacker_initial_deposit,
        "LP staked after donation mismatch"
    );
    ...
    assert_that!(user_vault_token_balance)
        .is_greater_than_or_equal_to(attacker_vault_token_balance);
...
pub fn vault_token_inflation_attack_full_dilute<Chain: CwEnv, Dex: DexInit>(
    ...
    let attacker_vault_token_balance = vault.vault_token_balance(attacker_addr)?;
    assert_eq!(
        attacker_vault_token_balance, 10u128,
        "Attacker vault token balance mismatch"
    );
    ...
    assert_eq!(
        lp_staked_after_donation,
        fully_dilute_donation + attacker_deposit,
        "LP staked after donation mismatch"
    );
    ...
    assert!(
        ...
        "Deposit did not fail as expected due to full dilution"
    );
