...
mod test {
    ...
    use abstract_core::objects::{PoolMetadata, AssetEntry};
    ...
    mod claims {
        ...
        fn test_query_claims() {
            ...
            let user = "user";
            CLAIMS.save(deps.as_mut().storage, user.to_string(), expected_claims).unwrap();
        ...
        fn test_query_all_claims() {
            ...
            CLAIMS.save(deps.as_mut().storage, user1.to_string(), &user1_claims.clone()).unwrap();
            CLAIMS.save(deps.as_mut().storage, user2.to_string(), &user2_claims.clone()).unwrap();
            CLAIMS.save(deps.as_mut().storage, user3.to_string(), &user3_claims.clone()).unwrap();
    ...
    mod unbonding {
        ...
        fn test_query_latest_unbonding() {
            ...
            LATEST_UNBONDING.save(deps.as_mut().storage, &expiration).unwrap();
    ...
    mod vault_token {
        ...
        fn test_query_assets_per_shares() {
            ...
            let assets_per_share = convert_to_assets(1000u128.into(), 100u128.into(), 1000u128.into());
            let mut config = default_config();
            config.vault_token = Addr::unchecked(TEST_VAULT_TOKEN);
            CONFIG.save(deps.as_mut().storage, &config).unwrap();
        
            let result = query_assets_per_shares(&app, deps.as_ref(), Some(1000u128.into())).unwrap();
