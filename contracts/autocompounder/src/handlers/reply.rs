...
use abstract_sdk::os::objects::{AssetEntry, AnsAsset, DexAssetPairing, LpToken};
use cosmwasm_std::{DepsMut, Env, Reply, Response, StdError, StdResult, Uint128, CosmosMsg, Addr, WasmMsg, to_binary};
use abstract_sdk::{ModuleInterface, Resolve};
use cw20::TokenInfoResponse;
use cw20_base::state::TokenInfo;
use cw20_base::msg::ExecuteMsg::{Mint, Burn, BurnFrom};
...
use crate::state::{Config, CONFIG, CACHED_USER_ADDR};
...
pub fn lp_provision_reply(
    ...
    let vault_token_info: TokenInfoResponse = deps.querier.query_wasm_smart(config.vault_token, &Cw20TokenInfo{ })?;
    let current_vault_supply =vault_token_info.total_supply;
    
    // 2) get the number of LP tokens minted in this transaction
    let lp_token_info: TokenInfoResponse = deps.querier.query_wasm_smart(config.vault_token, &Cw20TokenInfo{ })?;
    let new_lp_token_minted = lp_token_info.total_supply;


    let lp_token = AssetEntry::from(LpToken::from(config.pool_data));
    let vault_stake = query_stake(deps, app, lp_token); // TODO: THis might need to change to AssetEntry
    ...
    let mint_msg: CosmosMsg = WasmMsg::Execute {
        ...
        msg: to_binary(&Mint { recipient: user_address.to_string(), amount: mint_amount })?,
        funds: vec![],
    }.into();
